...
if __name__ == '__main__':
        ...
        os.chdir(stan_dir)
        os.system('make {}/luxpom'.format(curdir))
        ...
        for n in range(len(loci)):
            ...
            # get data and init dictionaries for stan
                data = luxpom_routines.get_stan_input(bsTot,bsMeth,D,bsEff,bsBEff,seqErr)
                start = timeit.default_timer()
                ...
                os.chdir(outdir)
                os.system('cp {}/luxpom .'.format(curdir))
                os.system('chmod 755 luxpom')
                os.system('./luxpom variational output_samples=1000 elbo_samples=1000 grad_samples=10 data file=data.json output diagnostic_file=diagnostics.csv > summary.txt')
                while 'COMPLETED' not in open('summary.txt','r').readlines()[-1]: os.system('./luxpom variational output_samples=1000 elbo_samples=1000 grad_samples=10 data file=data.json output diagnostic_file=diagnostics.csv > summary.txt')
                # compute bayes factor  

                bf=luxpom_routines.savagedickey(locus,ncols)
                fo1 = open(outfile1,'a')
